...
import { Plus, X, Loader2, Edit, Trash2, Check, RotateCcw, ArrowLeft, Home } from "lucide-react";
...
export default function AssessmentPage() {
  ...
  const handleCreateAspect = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/assessment/aspects', {
        ...
        body: JSON.stringify({ name: newAspectName, scale_type: newAspectScale, class_id: classId, date: selectedDate }),
      });
  ...
  const handleScoreChange = (studentId: string, aspectId: string, value: string | number) => {
    setScores(prev => ({ ...prev, [studentId]: { ...prev[studentId], [aspectId]: value } }));
  };
  ...
  const handleResetScores = () => {
    if (confirm("Apakah Anda yakin ingin mereset semua nilai yang telah diisi di form ini? Perubahan ini hanya akan diterapkan setelah Anda menekan tombol Simpan.")) {
      setScores({});
    }
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-md max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center mb-6">
            ...
            {editMode && (
              <button 
                ...
                className="flex-shrink-0 flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 font-semibold shadow-sm w-full sm:w-auto"
              >
                <Plus size={18} />
        ...
        {loading ? <p className="text-center py-12">Memuat data...</p> : (
            ...
            <table className="w-full text-sm">
                ...
                <tr>
                    <th className="p-4 font-semibold sticky left-0 bg-gray-100 z-10 w-1/4">Nama Siswa</th>
                    {aspects.map(aspect => (
                    <th key={aspect.id} className="p-4 font-semibold">
        ...
        {aspects.length > 0 && !loading && (
          <div className="mt-6 flex flex-col sm:flex-row justify-end items-center gap-4">
            {editMode ? (
              <>
                <button onClick={handleResetScores} className="px-6 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 flex items-center gap-2">
                  <RotateCcw size={18} />
                  Reset Nilai
                </button>
                <button onClick={handleSaveAllScores} className="px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 shadow-lg flex items-center gap-2 disabled:bg-green-400" disabled={isSavingScores}>
                  {isSavingScores ? <Loader2 size={20} className="animate-spin" /> : <Check size={20} />}
                  {isSavingScores ? 'Menyimpan...' : 'Simpan & Kunci Penilaian'}
                </button>
              </>
            ) : (
              <button onClick={() => setEditMode(true)} className="px-8 py-3 bg-yellow-500 text-white font-bold rounded-lg hover:bg-yellow-600 shadow-lg flex items-center gap-2">
                <Edit size={20} />
                Edit Penilaian
              </button>
            )}
