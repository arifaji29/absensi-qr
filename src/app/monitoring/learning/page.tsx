...
const formatDateDisplay = (dateString: string) => new Date(dateString).toLocaleDateString("id-ID", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'Asia/Jakarta'});
...
export default function LearningMonitoringPage() {
  ...
  // Fungsi untuk mengambil data jurnal dari API berdasarkan kelas dan rentang tanggal
  const fetchData = useCallback(async (class_id: string, startDate: string, endDate: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/monitoring/learning?class_id=${class_id}&start_date=${startDate}&end_date=${endDate}`);
      if (!res.ok) {
        throw new Error("Gagal memuat data jurnal");
      }
      const data = await res.json();
      data.sort((a: JournalLog, b: JournalLog) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  // Ambil data 7 hari terakhir saat komponen pertama kali dimuat
  useEffect(() => {
    if (classId) {
      const today = new Date();
      const endDate = formatDateToYYYYMMDD(new Date());
      const startDate = formatDateToYYYYMMDD(new Date(new Date().setDate(today.getDate() - 6)));
  ...
  // Fungsi untuk menangani perubahan tanggal dari pemilih tanggal
  const handleDateChange = (date: string) => {
    setSelectedDate(date);
    setViewTitle(`Laporan untuk tanggal: ${formatDateDisplay(date)}`);
    fetchData(classId, date, date); // Ambil data hanya untuk tanggal yang dipilih
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-md max-w-4xl mx-auto">
        ...
        {!classId ? (
            ...
            <>
                {/* Kontrol Tanggal */}
                <div className="mb-6">
                    <label htmlFor="journal-date" className="block text-sm font-medium text-gray-700 mb-1">Pilih Tanggal Laporan</label>
                ...
                <div>
                    <h2 className="text-xl font-semibold text-gray-700 mb-4">{viewTitle}</h2>
                    {journals.length > 0 ? (
                        <div className="space-y-6">
                        {journals.map((journal) => (
                          <div key={journal.id} className="border border-gray-200 rounded-lg overflow-hidden">
                            <div className="bg-gray-50 p-4 border-b">
                              <h3 className="font-bold text-lg text-gray-800">{formatDateDisplay(journal.date)}</h3>
                              <p className="text-sm text-gray-500">Diisi oleh: {journal.teacher_name}</p>
                            </div>
                    ...
                    <div className="text-center py-12 px-6 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-gray-500 mt-1">Tidak ada data jurnal pembelajaran yang diinput pada periode ini.</p>
                    </div>
                    )}
                </div>
                )}
