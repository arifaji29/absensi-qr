...
export default function AttendancePage() {
  ...
  async function handleReset() {
    if (!confirm(`Yakin mereset absensi kelas ${className} pada tanggal ${selectedDate}?`)) return;
    try {
  ...
  async function handleConfirmValidation() {
    if (!selectedValidatorId) return alert("Anda harus memilih satu pengajar sebagai validator.");
    try {
  ...
  return (
    <div className="p-6">
      ...
      {isValidated && ( <div className="mb-4 p-3 bg-green-100 border-l-4 border-green-500 text-green-800 rounded-md"><p className="font-bold">Absensi Tervalidasi</p><p>Presensi ini telah dikunci oleh <strong>{validatorName || "N/A"}</strong>, dan tidak dapat diubah lagi.</p></div> )}
      ...
      {loading ? (<p>Memuat data...</p>) : (
        <table className="w-full border-collapse border">
          <thead className="bg-gray-100"><tr><th className="border p-2">NIS</th><th className="border p-2">Nama</th><th className="border p-2 w-48">Status</th><th className="border p-2">Waktu Kehadiran</th></tr></thead>
          <tbody>
            {attendance.map((a) => (
              <tr key={a.student_id} className={a.status === 'Hadir' ? 'bg-green-100' : (a.status !== 'Belum Hadir' ? 'bg-yellow-100' : '')}>
                <td className="border p-2">{a.nis}</td><td className="border p-2">{a.name}</td>
                <td className="border p-2"><select value={a.status} onChange={(e) => handleStatusChange(a.student_id, e.target.value)} className="w-full p-2 border rounded bg-white disabled:bg-gray-100 disabled:cursor-not-allowed" disabled={isValidated}>{statusOptions.map(opt => (<option key={opt} value={opt}>{opt}</option>))}</select></td>
