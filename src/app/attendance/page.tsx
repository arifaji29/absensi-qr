...
export default function AttendancePage() {
  ...
  return (
    <div className="p-6">
      ...
      {/* Sisa dari JSX (pesan validasi, modal, tabel) tidak berubah */}
      {isValidated && (
      ...
      {isValidationModalOpen && (
          <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
              <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                  ...
                  <p className="text-gray-600 mb-6">Pilih pengajar yang bertanggung jawab atas validasi presensi ini.</p>
                  <div className="space-y-3 max-h-60 overflow-y-auto">
                      {classTeachers.map(teacher => (
                          <label key={teacher.id} className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                              <input type="radio" name="validator" value={teacher.id} checked={selectedValidatorId === teacher.id} onChange={(e) => setSelectedValidatorId(e.target.value)} className="h-5 w-5"/>
                              <span className="ml-4 font-medium">{teacher.name}</span>
                          </label>
                      ))}
                  </div>
      ...
      {isScannerOpen && (
        <div className="fixed inset-0 bg-black/80 flex flex-col items-center justify-center z-50"><div className="bg-white rounded-lg p-4 w-full max-w-sm"><h2 className="text-xl font-bold text-center mb-4">Arahkan Kamera ke QR Code</h2><div id="qr-reader-container"></div><button onClick={() => setIsScannerOpen(false)} className="mt-4 w-full bg-red-600 text-white py-2 rounded hover:bg-red-700">Tutup</button></div></div>
      )}

      {loading ? (<p>Memuat data untuk tanggal {selectedDate}...</p>) : (
