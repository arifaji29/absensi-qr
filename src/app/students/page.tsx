...
export default function StudentsPage() {
  ...
  // --- FUNGSI LOAD DATA DIPERBARUI ---
  async function loadData() {
    setLoading(true);
    try {
      const promises = [
        fetch("/api/classes"), // Selalu ambil semua kelas untuk dropdown
        fetch(`/api/students${activeClassId ? `?class_id=${activeClassId}` : ""}`), // Ambil siswa
      ];

      // Jika melihat kelas spesifik, ambil juga detail namanya
      if (classIdFromUrl) {
      ...
      // Proses nama kelas jika ada
      if (classIdFromUrl && responses[2] && responses[2].ok) {
        const classDetailData = await responses[2].json();
        setClassName(classDetailData.name);
      } else {
        setClassName(""); // Reset jika di halaman "Semua Kelas"
      }

    } catch (error) {
      if (error instanceof Error) console.error("Gagal memuat data:", error);
...
function downloadQR(student: Student) {
  // Gunakan nama kelas dari state jika ada, atau cari dari daftar
  ...
  root.render(
    <QRCodeCanvas
      value={JSON.stringify({ nis: student.nis, name: student.name, class: qrClassName })}
      size={220}
      includeMargin
    />
  );
  ...
  setTimeout(() => {
    ...
    ctx.font = "16px Arial";
    ctx.fillText(student.nis, finalCanvas.width / 2, y);
    y += 22;
    ctx.fillText(student.name, finalCanvas.width / 2, y);
    y += 22;
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    if (!editingId) return;
    try {
      // API untuk edit siswa biasanya tidak memerlukan class_id di URL
      const res = await fetch(`/api/students/${editingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form), // class_id sudah ada di dalam form state
      });
    ...
    <div className="p-6">
      ...
      {classIdFromUrl && (
        <div className="flex gap-3 mb-6">
          ...
          <button onClick={() => { resetForm(); setShowAddModal(true); }} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            Tambah Siswa
          </button>
        </div>
      )}
          ...
          <table className="w-full border-collapse border">
            <thead className="bg-gray-100">
              <tr>
                ...
                {classIdFromUrl && <th className="border p-2">Aksi</th>}
              </tr>
            </thead>
            <tbody>
              {students.length === 0 ? (
                <tr>
                  <td colSpan={classIdFromUrl ? 6 : 5} className="text-center p-4">
                    Belum ada data.
                  </td>
                </tr>
              ) : (
                students.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="border p-2">
                      {s.date_of_birth
                        ? new Date(s.date_of_birth).toLocaleDateString("id-ID")
                        : "-"}
                    </td>
                    <td className="border p-2">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center">
          <form
            onSubmit={handleAdd}
            className="bg-white p-6 rounded-lg space-y-3 w-full max-w-md"
          >
