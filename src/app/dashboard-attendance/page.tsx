...
function ParallelScannerModal({
  ...
  const handleClose = () => {
    if (isAutoSaving) {
      alert("Harap tunggu, data sedang disimpan...");
      return;
    }
    onClose();
  };
...
export default function AttendanceDashboard() {
  ...
  const handleOpenParallelScanner = async () => {
    ...
    try {
      ...
      setStudentsToScan(data.remainingStudents);
      setInitialAttendedCount(data.attendedCount);
  ...
  const handleSaveParallelAttendance = async (scannedStudents: Student[]) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorBody.message || "Gagal menyimpan data absensi.");
      }
    } catch (error) {
      console.error("Error in handleSaveParallelAttendance:", error);
      throw error;
    }
  };
